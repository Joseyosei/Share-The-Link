...
async function cascadeDeleteUser(supabase: any, targetUserId: string) {
...
async function handleAdminRemoveUser(supabase: any, body: any, adminUserId: string, res: VercelResponse) {
  ...
  await cascadeDeleteUser(supabase, targetUserId);
...
async function handleAdminDeleteStream(supabase: any, body: any, res: VercelResponse) {
  const { streamId } = body || {};
...
async function handleAdminDeleteRecording(supabase: any, body: any, res: VercelResponse) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (action) {
    ...
    try {
      switch (action) {
        case "remove-user":
          return await handleAdminRemoveUser(supabase, req.body, auth.userId, res);
        case "delete-stream":
          return await handleAdminDeleteStream(supabase, req.body, res);
        case "delete-recording":
          return await handleAdminDeleteRecording(supabase, req.body, res);
        default:
  ...
  try {
    await cascadeDeleteUser(supabase, auth.userId);

    const { error: deleteError } = await (supabase.auth as any).admin.deleteUser(auth.userId);
    if (deleteError) {
      console.error("Failed to delete auth user:", deleteError);
