...
-- Combined profile page data RPC (single call for profile + appearance)
CREATE OR REPLACE FUNCTION public.get_profile_page_data(lookup_username text)
RETURNS TABLE (
  user_id uuid, username text, full_name text, bio text, avatar_url text, social_links jsonb,
  theme text, background_type text, background_gradient text, background_color text,
  background_animation text, font_family text, title_color text, bio_color text, button_style text, button_color text
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT p.user_id, p.username, p.full_name, p.bio, p.avatar_url, p.social_links,
         COALESCE(a.theme, 'air'), a.background_type, a.background_gradient, a.background_color,
         a.background_animation, a.font_family, a.title_color, a.bio_color, a.button_style, a.button_color
  FROM public.profiles p
...
RETURNS TABLE (id uuid, title text, type text, duration integer, price numeric)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  ...
  ORDER BY bs.price ASC LIMIT 3;
$$;

CREATE OR REPLACE FUNCTION public.get_public_products(lookup_username text)
RETURNS TABLE (id uuid, name text, description text, price_cents integer, image_url text, category text, external_url text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
